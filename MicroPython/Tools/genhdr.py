...
def preprocess(in_file, out_file, incs, option):
    if os.name == 'posix':	
        ...
        cmd = "%s -I%s -o%s %s" % (option, inc_option, out_file, in_file)
    else:
        inc_option = " /I".join(incs)
        cmd = "%s /I%s /Fi%s %s" % (option, inc_option, out_file, in_file)
...
def collect_qstr(src, mpy_dir, qstr_dir):
    ...
    cmd = r"%s %s split qstr %s %s dummy" % (python, script, src, qstr_dir)
    subprocess.check_call(shlex.split(cmd, posix=False))

def cat_qstr(dst, mpy_dir, qstr_dir):
    python = "python"
    script = os.path.join(mpy_dir, "py", "makeqstrdefs.py")
    cmd = r"%s %s cat qstr dummy %s %s" % (python, script, qstr_dir, dst + ".tmp")
    subprocess.check_call(shlex.split(cmd, posix=False))
...
def main(args):
    ...
    with open(args.inf_file, 'r') as fd:
        ...
        for line in fd:
            ...
            if in_bld_section:
                tokens = line.split(':', 1)
                if len(tokens) == 2 and ((tokens[0].strip() == 'MSFT' and os.name == 'nt') or \
                   (tokens[0].strip() == 'GCC' and os.name == 'posix')): 
                    if '==' in tokens[1]:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', dest="pp", default='cl.exe', type=str, help='Preprocessor path')
    args = parser.parse_args()

    if os.name == 'posix':	
      PP_CMD = [args.pp, '-x', 'c', '-E', '-Wp,-w']
      TT_CMD = [args.pp, '--version']
    else:
      PP_CMD = [args.pp, '/nologo', '/P']
      TT_CMD = [args.pp]
    ...
    try:
        output = subprocess.check_output(TT_CMD, stderr=subprocess.STDOUT)
        print("Found:", output.decode("utf-8").split('\n')[0])
    except:
        print("ERROR: '%s' cannot run." % TT_CMD)
